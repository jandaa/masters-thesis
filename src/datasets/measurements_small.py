...
class SceneMeasurement:
    ...
    def __init__(
        ...
        # load raw data
        color_image = cv2.imread(str(color_image_name))
        ...
        color_image = cv2.resize(color_image, depth_resolution)

        # Convert depth map into point cloud
        self.points = get_point_cloud(depth_image, self.depth_intrinsics)
        ...
        mask = depth_image != 0
        self.point_colors = np.reshape(color_image[mask], [-1, 3])
